...
- **1 signalement réel** (`statuts_produit_magasin`) existe contre un EAN promotionnel (`La Laitière RAL nature sur lit de caramel 4x115g +2 offerts`, EAN `3023290003060`, magasin Auchan La Défense, manquant). **Correction (revue finale) :** cet EAN reste `a_qualifier` — aucune correspondance canonique fiable n'a été trouvée (voir "Rapport de correspondances canoniques") — donc la résolution à la lecture n'a rien à récupérer pour ce cas précis : le signalement reste attaché à son propre id, non résolu vers un canonique distinct. Comme ce produit ne figure plus dans le plan de vente actuel d'Auchan, il ne doit plus apparaître dans "produits manquants à travailler" tant qu'il n'est pas qualifié manuellement ; c'est la liste `manquants` elle-même qui doit être restreinte à l'assortiment actif (voir Modèle de données / correctif de revue finale), pas la résolution canonique. Tous les signalements futurs restent normalisés dès l'écriture (voir "Résolution à la lecture et à l'écriture").
...
**Distinction conservée** : le statut d'assortiment permanent est rattaché au canonique ; un futur constat d'exécution d'une opération promotionnelle spécifique (non construit en Phase 1 — aucune UI ne le permet aujourd'hui) resterait, lui, rattaché à l'EAN promotionnel réel s'il existait. Le signalement historique d'Auchan La Défense (EAN `3023290003060`, `a_qualifier`) n'a pas de canonique distinct vers lequel se résoudre ; il reste attaché à son propre id sans réécriture — c'est l'exclusion de "produits manquants à travailler" des produits hors assortiment actif (pas la résolution canonique) qui empêche ce cas de ressurgir comme une carte non éditable.
